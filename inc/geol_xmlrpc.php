...
function geodiv_liste_medias($args) {
	...
	$order = is_array($args['tri']) ? $args['tri'] : ['!date'];
...
function geodiv_lire_media($args) {
	...
	if (!intval($args['id_article']) > 0) {
		$erreur = _T('xmlrpc:erreur_identifiant', ['objet' => 'article']);
		return new IXR_Error(-32601, attribut_html($erreur));
	}

	$champs_demandes = is_array($args['champs_demandes']) ? $args['champs_demandes'] : [];
	...
	if ($id_secteur != $secteur_medias) {
		$erreur = _T('xmlrpc_geodiv:erreur_article_media', ['id_article' => $args['id_article']]);
		return new IXR_Error(-32601, attribut_html($erreur));
	}
	...
	if (count($champs_demandes) != 0) {
		foreach ($res['result'][0] as $champ => $valeur) {
			if (!in_array($champ, ['id_article']) && !in_array($champ, $champs_demandes)) {
